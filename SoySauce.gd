...
@onready var normal_sprite = $SoySauceNormal
@onready var pouring_sprite = $SoySaucePouring
...
func _physics_process(delta: float) -> void:
	...
	if is_pouring:
		pouring_timer += delta
		var progress_ratio = clamp(pouring_timer / pour_duration, 0.0, 1.0)
		pour_progress.value = progress_ratio * 100.0

		var pot1 = get_tree().get_first_node_in_group("pot1")
		if pot1 and pot1.has_method("set_soup1_transparency"):
			pot1.set_soup1_transparency(progress_ratio)
...
func start_pouring() -> void:
	...
	if not GameController.is_sake_finished:
		print("⛔ Sake must finish pouring before Soy Sauce can start.")
		return
	...
	print("🥢 Soy Sauce started pouring.")
...
func pouring_finished() -> void:
	...
	var pot1 = get_tree().get_first_node_in_group("pot1")
	if pot1 and pot1.has_method("set_soup1_transparency"):
		pot1.set_soup1_transparency(1.0)

	soup_created = true
	pour_progress.visible = false
	GameController.is_soy_sauce_finished = true
	print("✅ Soy Sauce finished pouring! Dashi can now pour.")
