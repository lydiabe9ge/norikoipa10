...
@onready var normal_sprite = $SakeNormal
@onready var pouring_sprite = $SakePouring
...
func _physics_process(delta: float) -> void:
	...
	if is_pouring:
		...
		if pot1 and pot1.has_method("set_soup_transparency"):
			pot1.set_soup_transparency(progress_ratio)
...
func start_pouring() -> void:
	if soup_created or soup_already_present():
		print("⚠️ Already created soup or soup present")
		return

	if not GameController.is_aburaage_dipped:
		print("⛔ Aburaage must be dipped before Sake can pour")
		return
	...
	print("🚰 Started pouring! Sound playing...")

	var pot1 = get_tree().get_first_node_in_group("pot1")
	if pot1 and pot1.has_node("SoupSprite"):
		pot1.get_node("SoupSprite").visible = true
...
func pouring_finished() -> void:
	...
	soup_created = true
	pour_progress.visible = false
	GameController.is_sake_finished = true
	print("✅ Sake finished pouring! Soy sauce can now pour.")
